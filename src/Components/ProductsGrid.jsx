...
const ProductsGrid = () => {
  return (
    <div className="bg-yellow-50 p-2">
      <h2 className="mt-20 text-black text-4xl font-bold text-center mb-20">
        Trending Products
      </h2>
      <div className="grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-3 gap-10 items-start">
        {products.map((product) => (
          <div
            key={product.id}
            className={`py-5 text-center transform duration-500 hover:-translate-y-2 cursor-pointer rounded-lg ${product.bg}`}
          >
            <img
              src={product.image}
              alt={product.title}
      ...
      <div className="my-20 text-center text-white border-t border-b border-yellow-700 p-2">
        <Link to={"/shop"}>
          <button className="px-10 font-bold py-3 rounded-sm">VIEW MORE</button>
        </Link>
      </div>
    </div>
  );
